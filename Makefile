...
GO_BUILD_ARGS=-tags "json1 fts5 sqlite_omit_load_extension" -a -installsuffix cgo -ldflags "-linkmode external -extldflags \"-static -lc\" -w -s"

GO_SRC=$(find . -type f -name "*.go" | grep -v vendor)
...
minimal-ldap-proxy:
	CGO_ENABLED=1 GOOS=linux go build ${GO_BUILD_ARGS} -o minimal-ldap-proxy .
